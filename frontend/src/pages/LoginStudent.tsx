import { useState } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import { useAuth } from '@/lib/auth';
import { motion } from 'framer-motion';
import { GraduationCap, AlertCircle, ShieldCheck } from 'lucide-react';
import Navbar from '@/components/layout/Navbar';
import AuthIllustration from '@/components/AuthIllustration';

export default function LoginStudent() {
  const { loginWithGoogle, loading } = useAuth();
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const [isVerifying, setIsVerifying] = useState(false);

  const handleGoogleLogin = async () => {
    setError('');
    setIsVerifying(true);
    try {
      await loginWithGoogle('student');
      setIsVerifying(false);
      navigate('/student/dashboard');
    } catch (err: any) {
      setIsVerifying(false);
      setError(err?.message || 'Google sign-in failed. Please use your official college Google account (e.g. name@gla.ac.in).');
    }
  };

  return (
    <div className="min-h-screen bg-background">
      <Navbar />
      <div className="container-main pt-24 pb-16">
        <div className="grid gap-10 lg:grid-cols-2 items-center">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="order-2 lg:order-1 w-full max-w-md mx-auto lg:mx-0"
          >
            <div className="bg-card border border-border rounded-2xl p-8 shadow-sm space-y-6">
              <div className="text-center">
                <GraduationCap className="h-10 w-10 text-primary mx-auto mb-3" />
                <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-primary/10 text-primary">
                  Student Portal
                </span>
                <h1 className="text-2xl font-bold text-foreground mt-2">
                  Student Sign In
                </h1>
                <p className="text-sm text-muted-foreground mt-1">
                  Authenticate instantly with your official university Google account
                </p>
              </div>

              {error && (
                <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/20 text-destructive text-sm flex items-start gap-2">
                  <AlertCircle className="h-4 w-4 flex-shrink-0 mt-0.5" />
                  <span>{error}</span>
                </div>
              )}

              {/* Google Login Option */}
              <button
                type="button"
                onClick={handleGoogleLogin}
                disabled={isVerifying || loading}
                className="w-full py-3 px-4 rounded-xl border border-border bg-card hover:bg-secondary/50 text-foreground text-sm font-semibold transition-colors flex items-center justify-center gap-3 disabled:opacity-50 shadow-sm"
              >
                <svg className="h-5 w-5" viewBox="0 0 24 24">
                  <path
                    fill="#4285F4"
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                  />
                  <path
                    fill="#34A853"
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  />
                  <path
                    fill="#FBBC05"
                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
                  />
                  <path
                    fill="#EA4335"
                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
                  />
                </svg>
                <span>{isVerifying ? 'Authenticating with Google...' : 'Continue with College Google Account'}</span>
              </button>

              <div className="p-4 rounded-xl bg-secondary/40 border border-border/60 text-xs space-y-2 text-muted-foreground">
                <div className="font-semibold text-foreground flex items-center gap-1.5">
                  <ShieldCheck className="h-4 w-4 text-emerald-500" />
                  <span>Secure University Identity Flow</span>
                </div>
                <ul className="space-y-1.5 list-disc pl-4 text-[11px] leading-relaxed">
                  <li>Direct OAuth authentication via your institutional Google Workspace.</li>
                  <li>Live camera ID verification and Polygon Soulbound Token (SBT) in dashboard.</li>
                  <li>Zero password management or manual form entries needed.</li>
                </ul>
              </div>

              <p className="text-center text-sm text-muted-foreground pt-1">
                New student?{' '}
                <Link to="/signup/student" className="text-primary hover:underline font-medium">
                  Create an account
                </Link>
              </p>
            </div>
          </motion.div>
          <AuthIllustration className="order-1 lg:order-2" />
        </div>
      </div>
    </div>
  );
}
