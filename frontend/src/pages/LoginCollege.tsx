...
export default function LoginCollege() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setIsSubmitting(false);
      setError(err?.message || 'Faculty Google login failed. Please ensure you are authorized for your institution.');
    }
  };
          ...
          >
            <div className="bg-card border border-border rounded-2xl p-8 shadow-sm space-y-6">
              <div className="text-center">
                ...
                <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-violet-500/10 text-violet-500">
              ...
              >
                ...
                <span>{isSubmitting ? 'Authenticating with Google...' : 'Continue with Institutional Google Account'}</span>
              </button>
              ...
              <div className="p-4 rounded-xl bg-secondary/40 border border-border/60 text-xs space-y-2 text-muted-foreground">
                <div className="font-semibold text-foreground flex items-center gap-1.5">
                  ...
                  <span>Institutional Authority Portal</span>
                </div>
                <ul className="space-y-1.5 list-disc pl-4 text-[11px] leading-relaxed">
                  <li>Single Sign-On using your university email domain.</li>
                  <li>Manage student identity verifications and review OCR logs.</li>
                  <li>Batch-mint Soulbound Tokens directly onto the Polygon blockchain.</li>
                </ul>
              </div>

              <p className="text-center text-sm text-muted-foreground pt-1">
                New faculty administrator?{' '}
                <Link to="/signup/college" className="text-primary hover:underline font-medium">
